...
colnames(avg.table) <- c("pat.id", "avg.gfr", "avg.fu.years") # assign colnames to table
avg.dt <- data.table(avg.table) # create data.table
head(avg.dt)
```
...
```{r}
CKD.stages.dt <- data.table("CKD stage" = c("V","IV","III","II","I"), 
...
Check `eGFR.dt` and `avg.dt` table:
```{r}
# eGFR.dt
setorderv(eGFR.dt, c("id","fu.years"), c(1,1))
stopifnot( all(eGFR.dt[, c("id", "fu.years")] == eGFR.dt[order(id, fu.years), fu.years, by = id] ))
